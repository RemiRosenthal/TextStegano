...
class MappingDictionary:
    """
    A mapping dictionary is defined uniquely by a single word-type and
    has a non-empty set of mappings between symbols and their
    bit-strings.
    """

    def __init__(self, mappings: Mappings, encode_spaces=True):
        input_mappings = dict(
            (string, bits) for string, bits in mappings)
        self.mappings = {}
...
class WordTypeDictionary:
    ...
    def append_word_type(self, input_dict: WTDict):
        """
        Add to this word-type dictionary the mappings of another
        dictionary. They each must have a unique word-type name and
        symbols which are unique to the dictionary.
        ...
        for key in input_keys:
            ...
            duplicate_values = self.contains_any_words_from_set(
                set(input_dict.get(key).mappings.items()), key)
            if duplicate_values.__len__() > 0:
                print("Dictionary contained duplicate values:")
            for duplicate_value in duplicate_values:
                print("{} ({})".format(
                    duplicate_value[0], duplicate_value[1]))
                del input_dict.get(key).mappings[duplicate_value[0]]

            if mapping_dict is None:
                self.wt_dict.update({key: input_dict.get(key)})
            else:
                mapping_dict.mappings.update(
                    input_dict.get(key).mappings)

    def contains_any_words_from_set(self, pairs: set,
                                    exclude_key=None) -> Set[
        Tuple[str, str]]:
        """
        Look through this word type dictionary. Return the subset of
        the input set of pairs whose value exists in this
        dictionary under any word-type.

        :param pairs: the set of key-value pairs to check
        :param exclude_key: the key to exclude from the search of the
        dictionary
    ...
    def remove_word_type(self, word_types: set):
        """
        Remove from this word-type dictionary every word list that is
        specified by a word-type name in the given set.

        Any invalid elements in the set are ignored.

        :param word_types: the set of word-types whose lists should be
    ...
    def remove_word(self, word_types: set):
        """
        Remove from this word-type dictionary every word that is
        specified by in the given set.

        Any invalid elements in the set are ignored.

        :param word_types: the set of words that should be removed
        from the dictionary
    ...
    def generate_state_definitions(self) -> list:
        """
        From this word-type dictionary, generate a list of distinct
        states identified uniquely by their word-type name.
...
def deserialise_dict(wt_dict: dict) -> WTDict:
    """
    Convert a serialised dict (of strings) to a WTDict (of mapping
    objects).
    ...
    for mapping_key in wt_dict.keys():
        ...
        mappings = {(x, Bits(bin=y)) for x, y in
                    mapping_dict.get("mappings").items()}
        deserialised_dict.update({
            mapping_key: MappingDictionary(
                mappings, mapping_dict.get("encode_spaces"))})
...
def save_dict(dictionary: WordTypeDictionary,
              dict_filename=DEFAULT_DICT_FILE):
    """
